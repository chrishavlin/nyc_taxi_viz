...
nx = 80 # number of bins in x (longitudinal) direction
ny = 100 # number of bins in y (latitudinal) direction

# the directory with the data
dir_base='./text_data/sub_sampling/'

# make the write directory 
write_dir='./data_man_' + str(nx) + '_' + str(ny)
try: 
...
#   create an empty TaxiCount array 
    ...
    DistTot=np.zeros((ny-1,nx-1)) # defined on cell centers
...
#   go back to start of file, loop again to read in lat/lon    
    ...
    #   read in each line separately, extract longitude, latitutde
        ...
        if len(line) == 19:
           longi=float(line[5])
           lati=float(line[6])
	   dist=float(line[4]) # distance traveled [miles]
           BigLat[indx,0]=lati
           BigLon[indx,0]=longi
	   BigDist[indx,0]=dist
           indx=indx+1
...
#   count the number of points within each lat/lon bin    
    for ix in range(nx-1):
	    ...
	    Dist=BigDist[BigLon[:,0]>xmin]
            LocLon=BigLon[BigLon[:,0]>xmin]
    
            LocLat=LocLat[LocLon[:,0]<xmax]
            Dist=Dist[LocLon[:,0]<xmax]
            LocLon=LocLon[LocLon[:,0]<xmax]
    
            LocLon=LocLon[LocLat[:,0]>ymin]
            Dist=Dist[LocLat[:,0]>ymin]
            LocLat=LocLat[LocLat[:,0]>ymin]
    
            LocLon=LocLon[LocLat[:,0]<ymax]
            Dist=Dist[LocLat[:,0]<ymax]
            LocLat=LocLat[LocLat[:,0]<ymax]


            LocLon=LocLon[Dist[:,0]<20]
            LocLat=LocLat[Dist[:,0]<20]
            Dist=Dist[Dist[:,0]<20]
    
            LocLon=LocLon[Dist[:,0]>0]
            LocLat=LocLat[Dist[:,0]>0]
            Dist=Dist[Dist[:,0]>0]

            nCount=len(LocLon)
            TaxiCount[iy,ix]=nCount
	    DistTot[iy,ix]=np.sum(Dist) # cumulative distance

    return TaxiCount,DistTot
"""-------------- END count_taxi_file ----------------------- """
...
DistTot=np.zeros((ny-1,nx-1)) # zero array to add counts to
...
for fn in os.listdir(dir_base):             # loop over directory contents 
     if os.path.isfile(dir_base+fn):        # is the current path obect a file?
      ...
      # distribute current file to lat/lon bins:
        TaxiCount2,DistTot2=count_taxi_file(fle,nx,ny,x,y,True) 
	...
	DistTot = DistTot + DistTot2        # aggregate distance traveled [miles]
        ifile = ifile+1                     # increment file counter

DistMean=np.divide(DistTot,TaxiCount+0.000001)
DistMean= DistMean * (TaxiCount > 0)
"""-----------------------------------------------
Save TaxiCount array and the lat/long bin arrays:
--------------------------------------------------"""

np.savetxt(write_dir+'/TaxiCount.txt', TaxiCount, delimiter=',')
np.savetxt(write_dir+'/TaxiMeanDist.txt', DistMean, delimiter=',')
